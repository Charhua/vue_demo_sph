...
this.getData();
  ...
  //  处理用户输入产品数
    changeSkuNum(e){
    //  输入值边界判断，并过滤非法值
      let tempVal=e.target.value * 1;
      if(isNaN(tempVal)||tempVal < 1){
        tempVal=1;
      }
      this.skuNum=parseInt(tempVal);
    },
  //  添加购物车操作
    async addShopCat(){
      // console.log('添加购物车')
  ...
  async getData(){
      try{
       await this.$store.dispatch("goodList", this.$route.params.skuId);
      }catch (err){
        this.$message({
          type:'error',
          message:'获取商品详情失败，请刷新！',
          duration:2000,
          center:true
        });
      }
